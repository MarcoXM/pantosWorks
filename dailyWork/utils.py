...
def get_night_data(contact_list_path):
    night = pd.read_excel(glob(contact_list_path)[-1])
    night_carrier_email = night.set_index('Carrier Code')['After HR'].to_dict()
    night_carriers_code = night['Carrier Code']
    return night_carriers_code, night_carrier_email